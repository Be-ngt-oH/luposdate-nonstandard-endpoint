...
public class ExtendedEndpoint {
	...
	public static void registerNonStandardContexts(String indexDir) {
		Endpoint.registerHandler("/nonstandard/sparql",
				new ExtendedQueryHandler(false));
		Endpoint.registerHandler("/nonstandard/sparql/info",
				new InfoQueryHandler(false));
		Endpoint.registerHandler("/nonstandard/sparql/graphs",
				new GraphsQueryHandler(false));
		Endpoint.registerHandler("/nonstandard/rif", new ExtendedQueryHandler(
				true));
		Endpoint.registerHandler("/nonstandard/rif/info", new InfoQueryHandler(
				true));
		Endpoint.registerHandler("/nonstandard/rif/graphs",
				new GraphsQueryHandler(true));
	}
}
